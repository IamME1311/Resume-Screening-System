...
def parse_jd_text(jd_text: str, jd_parser: JDParser):
...
def process_resumes(
    resume_files: list[UploadedFile],
    jd_data: dict,
    top_n: int,
    ...
    try:
        ...
        st.info(f"Parsing resume files...")
        resumes = []
        ...
        # extract data from parsed resumes
        ...
        if min_score is not None:
            scored_resumes = [
                r for r in scored_resumes if r["score"]["total_score"] >= min_score
            ]

        ranked_resumes = sorted(
            scored_resumes, key=lambda x: x["score"]["total_score"], reverse=True
        )
...
def main():
    ...
    st.markdown(
        '<h1 class="main-header"> Resume Screening System</h1>', unsafe_allow_html=True
    )
    st.markdown(
        '<p class="sub-header">Intelligent candidate matching powered by advanced algorithms</p>',
        unsafe_allow_html=True,
    )

    with st.spinner("Loading system componenets..."):
    ...
    with col1:
        st.markdown(
            '<h3 class="section-header">Job Description</h3>', unsafe_allow_html=True
        )
        ...
        if jd_input_method == "Paste Text":
            ...
            if jd_text:
                try:
                    ...
                    with st.expander(
                        "Job Description Parsed Successfully", expanded=False
                    ):
                        st.write(
                            f"**Required SKills:** {len(jd_Data['required_skills'])} skills identified"
                        )
                        if jd_Data["required_skills"]:
                            st.write(
                                f"Sample: {', '.join(list(jd_Data['required_skills'])[:5])}"
                            )
                        st.write(
                            f"**Preferred Skills:** {len(jd_Data['preferred_skills'])} skills identified"
                        )
                        if jd_Data["preferred_skills"]:
                            st.write(
                                f"Sample: {', '.join(list(jd_Data['preferred_skills'])[:5])}"
                            )
                        st.write(
                            f"**Minimum Experience:** {jd_Data['min_experience']} years"
                        )

                except Exception as e:
                    st.error(f"Error parsing job decription: {str(e)}")
            ...
            if jd_file:
                ...
                try:
                    jd_Data = jd_parser.parse_file(temp_path)
                    with st.expander(
                        "Job Description Parsed Successfully", expanded=False
                    ):
                        st.write(
    ...
    with col2:
        st.markdown(
            '<h3 class="section-header">Resume Files</h3>', unsafe_allow_html=True
        )
        resume_files = st.file_uploader(
            "Upload Resume Files:",
            type=["txt", "pdf", "docx", "doc"],
            accept_multiple_files=True,
            help="Upload multiple resumes (500-1000 supported). Supported formats: TXT, PDF, DOCX, DOC",
        )

        if resume_files:
            st.success(f"{len(resume_files)} resume filed uploaded")
    ...
    with col_btn2:
        process_button = st.button(
            "Start Screening Process", type="primary", use_container_width=True
        )
        ...
        if process_button:
            if not jd_Data:
                ...
                st.markdown("---")
                st.markdown(
                    '<h3 class="section-header">Processing</h3>', unsafe_allow_html=True
                )

                start_time = datetime.now() # override start_time
    ...
    if results:
        ...
        st.success(
            f"Screening completed successfully in {processing_time:.2f} seconds"
        )
        st.markdown("---")

        st.markdown(
            '<h3 class="section-header">Top Matching Candidates</h3>',
            unsafe_allow_html=True,
        )
        ...
        with col_m3:
            avg_score = sum(
                [r["score"]["total_score"] for r in results]
            ) / len(results)
        ...
        with col_d1:
            ...
            st.download_button(
                ...
                data=csv,
                file_name=f"results_{datetime.now().strftime("%d%m%Y_%H%M%S")}.csv",
                mime="text/csv",
        ...
        with col_d2:
            ...
            st.download_button(
                ...
                data=json_data,
                file_name=f"results_{datetime.now().strftime("%d%m%Y_%H%M%S")}.json",
            ...
            with zipfile.ZipFile(
                ...
                for idx, result in enumerate(results, 1):
                    if "file_object" in result["resume"]:
                        ...
                        file_name = result["resume"].get(
                            "filename", f"resume_{idx}.pdf"
                        )

                        ranked_filename = f"{idx:02d}_{file_name}"
                        zip_file.writestr(
                            ranked_filename, file_obj.getvalue()
                        )

            st.download_button(
                label="Download All Resumes (ZIP)",
                data=zip_buffer.getvalue(),
                file_name=f"top_resumes_{datetime.now().strftime("%d%m%Y_%H%M%S")}.zip",
        ...
        st.markdown("---")
        st.markdown(
            '<h3 class="section-header">Detailed Candidate Profiles</h3>',
            unsafe_allow_html=True,
        )
        ...
        for idx, result in enumerate(
            ...
            with st.expander(
                f"Rank #{idx} - {resume_filename} - Match Score: {result['score']['total_score']:.1f}%"
            ):
                ...
                with col_detail1:
                    ...
                    st.write(
                        f"Email: {result['resume'].get('email', 'N/A')}"
                    )
                    st.write(
                        f"Phone: {result['resume'].get('phone', 'N/A')}"
                    )
                ...
                with col_detail2:
                    ...
                    st.write(
                        f"Required Skills Match: {result['score']['required_skills_score']:.1f}%"
                    )
                    st.write(
                        f"Preferred Skills Match: {result['score']['preferred_skills_score']:.1f}%"
                    )
                    st.write(
                        f"Experience Match: {result['score']['experience_score']:.1f}%"
                    )
                    st.write(
                        f"Keyword Match: {result['score']['keyword_score']:.1f}%"
                    )
                ...
                if result["score"]["matched_required_skills"]:
                    st.write(
                        ", ".join(
                            result["score"]["matched_required_skills"]
                        )
                    )
                else:
                ...
                if result["score"]["missing_required_skills"]:
                    st.write(
                        ", ".join(
                            result["score"]["missing_required_skills"]
                        )
                    )
                else:
