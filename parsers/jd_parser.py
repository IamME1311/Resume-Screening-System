...
class JDParser:
    ...
    def parse_file(self, file_path_str: str) -> dict:
        """
        Parse job descriptions from file (supports .txt, .pdf, .docx)

        Args:
            file_path_str: path to the JD file
        ...
        try:
            ...
            raise Exception(f"Error reading JD file")
    ...
    def _split_sections(self, text: str) -> tuple:
        ...
        for keyword in self.required_keyords:
        ...
        for keyword in self.preferred_keywords:
            pattern = rf"{keyword}[:\s]+(.*?)(?={"|".join(self.required_keyords)}|$)"
            match = re.search(pattern, text, re.IGNORECASE | re.DOTALL)
            if match:
    ...
    def _extract_skills(self, text: str) -> set[str]:
        ...
        skill_patterns = [
            ...
            # java ecosystem
            r"\b(spring|spring\s*boot|spring\s*mvc|spring\s*cloud|hibernate|jpa|jdbc|j2ee|jee|java\s*ee|servlets|jsp|jsf|struts|maven|gadle|tomcat|jboss|wildfly|weblogic|websphere)\b",
            # python ecosystem
            ...
            # databases
            r"\b(mysql|postgresql|postgres|mongodb|mongo|oracle|sql\s*server|mssql|sqlite|redis|cassandra|dynamodb|couchdb|mariadb|neo4j|elasticsearch|elastic|sql|nosql|plsql|pl/sql|t-sql|rdbms|database)\b",
            # cloud platforms
            r"\b(aws|amazon\s*web\s*services|azure|microsoft\s*azure|gcp|google\s*cloud\s*platform|cloud|heroku|digitalocean|linode|vercel|netlify)\b",
            # AWS services
